...
#        self.image_dir = '/home/' + usr + '/Workspace/Datasets/' + dataset + '/'
        ...
        idx_valid_writers = sorted(np.random.choice([x for x in idx_writers if x not in idx_train_writers], num_test_writers, replace=False))
        ...
        self.valid_lab = np.array([float(line.split(' ')[2].strip('\n')) for line in self.valid_lines])
...
#                    validation_data=datagen.next_val(), nb_val_samples=552*num_test_writers)

pred = model.predict_generator(generator=datagen.next_train(), val_samples = 552*num_train_writers)
acc = compute_accuracy(pred, datagen.train_lab)
print('* Accuracy on train set: %0.2f%%' % (100 * acc))
pred = model.predict_generator(generator=datagen.next_val(), val_samples = 552*num_test_writers)
