...
class DataGenerator(object):    
    ...
    def __init__(self, dataset, tot_writers, num_train_writers, 
        ...
        nsamples = 6#3#1#69#138#276 #24C2
...
def create_base_network_phoc(input_shape):
    '''Base network to be shared (eq. to feature extraction).
    '''
...
#    seq.add(Convolution2D(16, 3, 3,  activation='relu', border_mode='same', input_shape=input_shape, name='block1_conv1'))
#    seq.add(Convolution2D(16, 3, 3, activation='relu', border_mode='same', name='block1_conv2'))
#    seq.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='block1_pool', dim_ordering='tf'))
    
    seq.add(Convolution2D(64, 3, 3,  activation='relu', border_mode='same', input_shape=input_shape, name='block1_conv1'))
    
    seq.add(Convolution2D(64, 3, 3, activation='relu', border_mode='same', name='block1_conv2'))
#    seq.add(BatchNormalization(epsilon=1e-06, mode=0, axis=1, momentum=0.9))
    ...
    seq.add(Convolution2D(128, 3, 3, activation='relu', border_mode='same', name='block2_conv2'))
#    seq.add(BatchNormalization(epsilon=1e-06, mode=0, axis=1, momentum=0.9))
    seq.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='block2_pool', dim_ordering='tf'))
    
    seq.add(Convolution2D(256, 3, 3, activation='relu', border_mode='same', name='block3_conv1'))
#    seq.add(Convolution2D(256, 3, 3, activation='relu', border_mode='same', name='block3_conv2'))
#    seq.add(Convolution2D(256, 3, 3, activation='relu', border_mode='same', name='block3_conv3'))
#    seq.add(ZeroPadding2D((1,1)))
#    seq.add(Convolution2D(256, 3, 3, activation='relu', border_mode='same', name='block3_conv3'))
    seq.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='block3_pool', dim_ordering='tf'))
#    seq.add(ZeroPadding2D((1,1)))
#    seq.add(Convolution2D(256, 3, 3, activation='relu', border_mode='same', name='block4_conv1'))
#    seq.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='block4_pool', dim_ordering='tf'))
    
    seq.add(Dropout(0.3))
    
    seq.add(ZeroPadding2D((1,1)))
    seq.add(Convolution2D(512, 3, 3, activation='relu', border_mode='valid', name='block5_conv1'))
#    seq.add(Convolution2D(512, 3, 3, activation='relu', border_mode='valid', name='block5_conv2'))# can remove this
 #   seq.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='block5_pool', dim_ordering='tf'))
    
    seq.add(Flatten(name='flatten'))
    
    seq.add(Dense(1024, activation='relu', name='fc1'))
    seq.add(Dropout(0.5))
#    seq.add(Dense(256, activation='relu', name='fc2'))
#    seq.add(Dropout(0.2))
    seq.add(Dense(512, activation='relu', name='fc2'))
...
#    model.add(Dropout(0.3))# added extra
    ...
    model.add(MaxPooling2D((3,3), strides=(2, 2)))
    model.add(Dropout(0.3))# added extra
#    model.add(SpatialPyramidPooling([1, 2, 4])) # added Spatial Pyramid Pooling
    model.add(Flatten(name='flatten'))
    model.add(Dense(4096, W_regularizer=l2(0.0005), activation='relu', init='glorot_uniform'))
    model.add(Dropout(0.5))
    model.add(Dense(1000, W_regularizer=l2(0.0005), activation='relu', init='glorot_uniform')) # softmax changed to relu
    return model
...
dataset = 'GPDS960'
tot_writers = 4000
num_train_writers = 3500
num_test_writers = 500
batch_size = 10#276#69#138#276
...
# network definition
base_network = create_base_network_phoc(input_shape)
...
# train
rms = RMSprop(lr=0.00001, rho=0.9, epsilon=1e-08)
...
#model.load_weights('/home/sounak/Documents/python_learning/siamese-signature-verification/weights_1574.hdf5')
...
checkpointer = ModelCheckpoint(filepath="/home/sounak/Documents/python_learning/siamese-signature-verification/weights.hdf5", verbose=1, save_best_only=True)

model.fit_generator(generator=datagen.next_train(), samples_per_epoch=datagen.samples_per_train, nb_epoch=nb_epoch,
                    validation_data=datagen.next_val(), nb_val_samples=12*num_test_writers, callbacks=[checkpointer])

model.load_weights('/home/sounak/Documents/python_learning/siamese-signature-verification/weights.hdf5')
pred = model.predict_generator(generator=datagen.next_test(), val_samples = 12*num_test_writers)
