...
from read_data_set import read_data_cedar
...
idx_writers = list(range(55))

#idx_train_writers = sorted(rn.sample(list(range(4000)), num_train_writers))
idx_train_writers = sorted(np.random.choice(idx_writers, num_train_writers, replace=False))

train_pairs, train_labels = read_data_cedar(idx_train_writers, input_shape, height=height, width=width)

print('Done.')

print('Loading test data...', end="", flush=True)

#idx_test_writers = sorted(rn.sample(set(list(range(4000))) - set(idx_train_writers), num_test_writers))
...
#idx_test_writers = sorted(np.random.choice(set(list(range(4000))) - set(idx_train_writers), num_test_writers, replace=False))

test_pairs, test_labels = read_data_cedar(idx_test_writers, input_shape, height=height, width=width)
